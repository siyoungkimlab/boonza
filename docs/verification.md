...
| viparr force fields (`parameterize`, `merge_forcefields`) | viparr (built against msys) | every table term by term on 11 systems: CHARMM36m (solvated), ff14SB, ff19SB, ff99SB-ILDN, DES-Amber, lipid + ion NBFIX, TIP4P-Ew, TIP5P, a `-m` merge, first-match priority; energies against OpenMM's `amber14-all.xml` and `charmm36_2024.xml` |
